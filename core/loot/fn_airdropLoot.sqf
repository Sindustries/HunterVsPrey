	...
	private ["_drop","_dropRate","_weapon","_mag1","_mag2","_item","_clothes","_backpack","_nvRoll","_spRoll","_spClothes","_nvg","_sns"];
	...
	for "_i" from 0 to _dropRate do {
		...
		if (HVPGameType isEqualTo 2 || HVPGameType isEqualTo 3) then {
			if (((Sinspawn_lootChance select 6)*2) > (random 100)) then {
				_drop addItemCargoGlobal [_spClothes, floor (random 2)];
			};
			if (((Sinspawn_lootChance select 7)*2) > (random 100)) then {
				_drop addItemCargoGlobal [_nvg, floor (random 2)];
			};
			if (((Sinspawn_lootChance select 8)*2) > (random 100)) then {
